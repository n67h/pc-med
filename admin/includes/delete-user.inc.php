<?php
    ...
    if(isset($_GET['id'])) {
        ...
        $sql1 = "UPDATE users SET is_deleted = 1 WHERE user_id = $id;";
        ...
        if(mysqli_num_rows($result_service) > 0){
            $sql5 = "UPDATE service_reservation SET is_deleted = 1 WHERE user_id = $id;";
        ...
        if(mysqli_num_rows($result_feedback) > 0){
            $sql6 = "UPDATE feedback SET is_deleted = 1 WHERE user_id = $id;";
        ...
        if(mysqli_num_rows($result_order_details) > 0){
            $sql2 = "UPDATE order_details SET is_deleted = 1 WHERE user_id = $id;";
            if(mysqli_query($conn, $sql2)){
                ...
                if(mysqli_num_rows($result_orders) > 0){
                    $sql3 = "UPDATE orders SET is_deleted = 1 WHERE order_details_id = $order_details_id;";
                ...
                if(mysqli_num_rows($result_payment) > 0){
                    $sql4 = "UPDATE payment_details SET is_deleted = 1 WHERE order_details_id = $order_details_id;";
